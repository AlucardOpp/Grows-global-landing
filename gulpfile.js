...
const cleanJson = () => {
  return del('source/json');
};

const jsonMerge = () => {
  return gulp.src('source/localization/*.json')
    .pipe(jsonConcat('ALL_languages.json',function(data){
      return new Buffer(JSON.stringify(data));
    }))
    .pipe(gulp.dest('source/json'));
};

const pugToHtml = () => {
  dataFromFile = JSON.parse(fs.readFileSync('source/json/ALL_languages.json'));
...
const pugToHtml2 = () => {
  return gulp.src('source/pug/localized-files/**/*.pug')
      .pipe(plumber())
      .pipe(pug({
        pretty: true,
        locals: dataFromFile || {}
      ...
      .pipe(cached('pug'))
      .pipe(gulp.dest('build/localized-files'));
};
...
const start = gulp.series(cleanJson, clean, svgo, copy, css, sprite, js, js2, jsonMerge, pugToHtml, pugToHtml2, syncServer);

const build = gulp.series(clean, svgo, copy, css, sprite, js, js2, pugToHtml, pugToHtml2, optimizeImages);
